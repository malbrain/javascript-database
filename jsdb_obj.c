...
value_t lookupDoc(value_t obj, value_t name) {
    uint32_t *table = (uint32_t *)(obj.document->names + obj.document->count * 2);
    uint32_t h, start, idx;
    value_t v;
    
    h = start = hashStr(name) % obj.document->capacity;

    while ((idx = table[h])) {
        value_t key = obj.document->names[idx - 1];

        if (key.rebaseptr)
            key.rebase = obj.rebase - obj.document->base + key.offset;

        if (key.aux == name.aux)
            if (!memcmp(key.str, name.str, name.aux)) {
                v = obj.document->names[obj.document->count + idx - 1];
                if (v.rebaseptr)
                    v.rebase = obj.rebase - obj.document->base + v.offset;
                return v;
            }

        h = (h+1) % obj.document->capacity;
