 ...
 */
function Cipher(input, w) {    // main Cipher function
...
function SubBytes(s, Nb) {    // apply SBox to state S
  for (var r=0; r<4; r++) {
...
function MixColumns(s, Nb) {   // combine bytes of each col of state S
  for (var c=0; c<4; c++) {
    ...
    var b = new Array(4);  // 'b' is in GF(2^8)
    for (var i=0; i<4; i++) {
      a[i] = s[i][c];
      b[i] = s[i][c]&0x80 ? s[i][c]<<1 ^ 0x011b : s[i][c]<<1;
    }
    // a[n] ^ b[n] is in GF(2^8)
...
function KeyExpansion(key) {  // generate Key Schedule (byte-array Nr+1 x Nb) from Key
  var Nb = 4;            // block size (in words): no of columns in state (fixed at 4 for AES)
...
function AESEncryptCtr(plaintext, password, nBits) {
  ...
  // initialise counter block (NIST SP800-38A.2): millisecond time-stamp for nonce in 1st 8 bytes,
  ...
  for (var i=0; i<4; i++) counterBlock[i+4] = (nonce >>> (i * 8 + 32)) & 0xff;//(nonce/0x100000000 >>> i*8) & 0xff; 
  ...
  for (var b=0; b<blockCount; b++) {
    ...
    for (var c=0; c<4; c++) counterBlock[15-c-4] = (b >>> (c*8 + 32)) & 0xff;//(b/0x100000000 >>> c*8);
...
//  ciphertext = ciphertext.split('-');  // split ciphertext into array of block-length strings 
  ciphertext = ChunkStr(ciphertext, 16);
  ...
  for (var b=1; b<ciphertext.length; b++) {
    ...
    for (var c=0; c<4; c++) counterBlock[15-c-4] = (((b >>> 32) - 1) >>> c*8) & 0xff;//((b/0x100000000-1) >>> c*8) & 0xff;
...
if (decryptedText != plainText)
    print( "ERROR: bad result: expected " + plainText + " but got " + decryptedText);
else
	print("OK");
