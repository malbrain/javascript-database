...
function AESEncryptCtr(plaintext, password, nBits) {
  ...
  // initialise counter block millisecond time-stamp for nonce in 1st 8 bytes,
...
function AESDecryptCtr(ciphertext, password, nBits) {
  ...
  key = key.concat(key.slice(0, nBytes-blockSize));  // key is now 16/24/32 bytes long

  var keySchedule = KeyExpansion(key);

  ciphertext = ChunkStr(ciphertext, blockSize);  // split ciphertext into array of block-length strings 
